 ...
 public:
  ...
  BipartiteGraph(const GRAPH& originalGraph)
    ...
    std::vector<Vertex> unassignedVertices =
        Vector::id<Vertex>(graph.numVertices());
    while (!unassignedVertices.empty()) {
      ...
      IndexedSet<false, Vertex>& otherSide =
          leftSide.contains(from) ? rightSide : leftSide;
  ...
  inline bool checkSideContainsOnlyVerticesInGraph(
      IndexedSet<false, Vertex>& side) {
...
inline std::vector<Dinic::CutEdge> maximumBipartiteMatching(
    BipartiteGraph& data) {
