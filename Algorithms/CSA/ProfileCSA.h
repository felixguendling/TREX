...
private:
  ...
  struct ProfileElement {
    ProfileElement(int departureTime = never, int arrivalTime = never,
                   ...
                   ConnectionId exit = ConnectionId(-1))
        : arrivalTime(arrivalTime), departureTime(departureTime), enter(enter),
          exit(exit) {}
...
public:
  ProfileCSA(Data &data, const Profiler &profilerTemplate = Profiler())
      : data(data), reverseTransferGraph(data.transferGraph),
        sourceStop(noStop), targetStop(noStop),
        ...
        sourceDominationIndex(0), profiler(profilerTemplate) {
