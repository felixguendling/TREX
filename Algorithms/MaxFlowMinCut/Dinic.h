...
public:
  Dinic(DynamicFlowGraph &&dynamicGraph)
      : residualFlows(std::vector<int>(dynamicGraph.numEdges(), 0)),
        ...
        targetVertices(dynamicGraph.numVertices()), capacityLimit(1 << 16),
        flow(0) {
...
private:
  ...
  template <bool FORWARD> inline bool computeDistances() {
    for (const Vertex vertex : activeVertices) {
      distances[vertex] = intMax;
    }
    currentQueue.clear();
  ...
  template <bool INCREASING_DISTANCE> inline size_t findBlockingPath() {
    ...
    AssertMsg(usedEdges.empty() || targetVertices.contains(
                                       graph.get(ToVertex, usedEdges.back())),
    ...
    for (const Edge edge : usedEdges) {
      AssertMsg(residualFlows[edge] > 0, "An edge with residual flow "
                                             << residualFlows[edge]
                                             << " was used!");
  ...
  template <bool FORWARD> inline Cut getCut() {
    Cut cut;
    size_t cutSize = 0;
    ...
    AssertMsg(cutSize == flow, "Maximum flow is " << flow << " but cut size is "
                                                  << cutSize << "!");
    return cut;
  }
