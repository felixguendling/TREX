...
template <typename PROFILER = NoProfiler> class QueryAStar {
public:
...
public:
  struct DijkstraLabel {
    DijkstraLabel()
        : arrivalTime(never), parentDepartureTime(never), numberOfTrips(255),
          routeId(noRouteId), parentStop(noStop), parentIndex(-1),
          bestTravelTime(0), bestNumTrips(0) {}
    ...
    DijkstraLabel(LABEL &parentLabel, int travelTime)
        : arrivalTime(parentLabel.arrivalTime + travelTime),
          ...
          numberOfTrips(parentLabel.numberOfTrips), routeId(noRouteId),
          parentStop(parentLabel.parentStop),
          parentIndex(parentLabel.parentIndex), bestTravelTime(0),
          bestNumTrips(0) {}
    ...
    DijkstraLabel(const DijkstraLabel &other)
        : arrivalTime(other.arrivalTime + other.bestTravelTime),
          ...
          routeId(other.routeId), parentStop(other.parentStop),
          parentIndex(other.parentIndex), bestTravelTime(other.bestTravelTime),
    ...
    DijkstraLabel(int newArrivalTime, int newParentDepartureTime,
        ...
        : arrivalTime(newArrivalTime),
          ...
          numberOfTrips(newNumberOfTrips), routeId(newRouteId),
          parentStop(parentStop), parentIndex(parentIndex),
          bestTravelTime(newBestTravelTime), bestNumTrips(newBestNumTrips) {}

    DijkstraLabel(int departureTime, StopId sourceStop)
        : arrivalTime(departureTime), parentDepartureTime(departureTime),
          numberOfTrips(0), routeId(noRouteId), parentStop(sourceStop),
          parentIndex(0), bestTravelTime(0), bestNumTrips(0) {}
...
public:
  QueryAStar(Data &data)
      : data(data), sourceStop(noVertex), targetStop(noVertex),
        sourceDepartureTime(0), queryGraph(),
        queue(data.maxNumVerticesAndNumEdgesInTP().first), left(0), right(0),
