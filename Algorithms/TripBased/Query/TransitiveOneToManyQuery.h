...
class TransitiveOneToManyQuery {
 public:
  using Profiler = PROFILER;
  using Type = TransitiveOneToManyQuery<Profiler>;
 ...
 public:
  ...
  inline void run(const StopId source, const int departureTime,
                  const std::vector<StopId> &targets, const std::uint16_t cell,
    ...
    for (auto t : targets) {
      std::uint32_t target(t);
      targetSet.insert(target);
      targetLabels[target] = std::vector<TargetLabel>(1);
      targetLabels[target][0] = TargetLabel();
    }
  ...
  inline std::vector<RAPTOR::Journey> getJourneys(
    ...
    for (const TargetLabel &label :
         targetLabels[static_cast<std::uint32_t>(target)]) {
      if (label.arrivalTime >= bestArrivalTime) continue;
      bestArrivalTime = label.arrivalTime;
      result.emplace_back(getJourney(target, label));
    }
    return result;
  }
  ...
  //             continue;
  //         if ((result.size() >= 1) && (result.back().arrivalTime ==
  //         targetLabels[i].arrivalTime))
  //             continue;
 ...
 private:
  ...
  inline void computeInitialAndFinalTransfers() noexcept {
    ...
    */
    ...
    if (targetSet.contains(sourceStop))
        addTargetLabel(sourceStop, sourceDepartureTime);
    ...
    reverseTransferGraph.get(TravelTime, edge)); transferToTarget[stop] =
    ...
    while (roundBegin < roundEnd && currentRoundNumber < 16) {
      ...
      for (size_t i = roundBegin; i < roundEnd; i++) {
        ...
        for (StopEventId j = label.begin; j < label.end; j++) {
          ...
          // StopId thisStop = data.arrivalEvents[j].stop;
          // if (data.arrivalEvents[j].arrivalTime >= minArrivalTime)
          //     break;
          for (const Edge edge : data.raptorData.transferGraph.edgesFrom(
                   data.arrivalEvents[j].stop)) {
            const Vertex perFootpathReachableStop =
                data.raptorData.transferGraph.get(ToVertex, edge);
            const int timeToTarget =
                data.raptorData.transferGraph.get(TravelTime, edge);
            if (targetSet.contains(
                    static_cast<std::uint32_t>(perFootpathReachableStop))) {
              addTargetLabel(StopId(perFootpathReachableStop),
  ...
  inline void enqueue(const Edge edge, const size_t parent) noexcept {
    ...
    if ((data.cellIds[toStop] >> currentLevel) != currentCell) return;
  ...
  inline void addTargetLabel(const StopId target, const int newArrivalTime,
                             const u_int32_t parent = -1) noexcept {
    profiler.countMetric(METRIC_ADD_JOURNEYS);
    if (newArrivalTime <
        targetLabels[static_cast<std::uint32_t>(target)].back().arrivalTime) {
      targetLabels[static_cast<std::uint32_t>(target)].back() =
          TargetLabel(newArrivalTime, parent);
      /* minArrivalTime = newArrivalTime; */
    }
  }

  inline RAPTOR::Journey getJourney(
      const StopId target, const TargetLabel &targetLabel) const noexcept {
    RAPTOR::Journey result;
    u_int32_t parent = targetLabel.parent;
    if (parent == u_int32_t(-1)) {
      result.emplace_back(sourceStop, target, sourceDepartureTime,
    ...
    while (parent != u_int32_t(-1)) {
      ...
      std::tie(arrivalStopEvent, edge) =
          (departureStopEvent == noStopEvent)
              ? getParent(target, label, targetLabel)
  ...
  inline std::pair<StopEventId, Edge> getParent(
      const StopId thisTarget, const TripLabel &parentLabel,
      const TargetLabel &targetLabel) const noexcept {
    // Final transfer to thisTarget may start exactly at parentLabel.end if it
    // has length 0
    ...
    for (StopEventId i = parentLabel.begin; i < end; i++) {
      for (const Edge edge : reverseTransferGraph.edgesFrom(thisTarget)) {
        const Vertex stop = reverseTransferGraph.get(ToVertex, edge);
        const int timeToTarget = reverseTransferGraph.get(TravelTime, edge);
        if (stop == data.arrivalEvents[i].stop &&
            data.arrivalEvents[i].arrivalTime + timeToTarget ==
                targetLabel.arrivalTime)
 ...
 private:
  ...
  std::unordered_map<std::uint32_t, std::vector<TargetLabel>> targetLabels;
  // int minArrivalTime;
