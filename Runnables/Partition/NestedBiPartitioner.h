...
public:
    ...
    Partitioner(
        ...
        , cellIds(vertexWeight.size(), 0)
        , mapping(vertexWeight.size(), 0)
        , numLevels(numLevels) {};

    void setNumLevels(size_t newNumLevels)
    {
    ...
    void clear()
    {
        cellIds.assign(vertexWeight.size(), 0);
    }
...
private:
    ...
    void runBipartitionForLevel(size_t currentLevel)
    {
        ...
        // helper function, returns true iff on this currentLevel, the given cellId is set
        ...
        for (int i(0); i < (1 << (numLevels - currentLevel - 1)); ++i) {
            if (VERBOSE)
                std::cout << "[Level] " << (currentLevel + 1) << " [cellId] " << std::bitset<32>(i) << std::endl;
            ...
            for (size_t j(0); j < verticesOfCell.size(); ++j) {
                ...
                for (size_t toIndex(toAdj[node]); toIndex < toAdj[node + 1]; ++toIndex) {
            ...
            if (VERBOSE) {
                ...
                std::cout << "# of vertices:  " << numVerticesSubgraph << ", weight: " << totalWeight << std::endl;
            }
            ...
            std::copy(subgraphToAdjVec.begin(), subgraphToAdjVec.end(), subgraphToAdj);
            std::copy(subgraphToVertexVec.begin(), subgraphToVertexVec.end(), subgraphToVertex);
            std::copy(subgraphToWeightVec.begin(), subgraphToWeightVec.end(), subgraphToWeight);
            std::copy(subgraphVertexWeightVec.begin(), subgraphVertexWeightVec.end(), subgraphVertexWeight);
